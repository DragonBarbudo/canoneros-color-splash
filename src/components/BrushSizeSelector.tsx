    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {sizes.map((size) => (
          <button
            key={size}
            className={`h-16 border-3 transition-all duration-200 hover:scale-105 flex items-center justify-center  rounded-lg ${
              brushSize === size
                ? "border-brand-cyan bg-brand-purple scale-105"
                : "border-brand-cyan bg-white/25 hover:border-brand-white"
            }`}
